    ...
    <div class="header">
      ...
      <div class="header__topbar-classic">
        ...
        <div class="topbar topbar--classic">
          ...
          @if(Auth::check())
          <div class="topbar__item-text"><a class="topbar__link" href="{{url('/user-order-history')}}">Track Order</a>
          @else
          <div class="topbar__item-text"><a class="topbar__link" href="{{url('/user-order-history')}}">Track Order</a>
          @endif
          </div>
...
<!--                  <div class="main-menu__submenu">
                          ...
                          <div class="megamenu">
                            <div class="row">
                              @foreach($sub_doctor1 as $r)
                                <?php  
                                  $sub1_doctor = DB::table('categories')->whereNotIn('categories_id',[438,439])->where('sub_parent_id',$r->categories_id)->where('sub_parent_id','!=' , null)->where('parent_id','!=' , null)->where('sub_sub_parent_id',  null)->get();  
                                ?>
                              <div class="col-2">
                                <ul class="megamenu__links megamenu-links megamenu-links--root">
                          ...
                          <div class="megamenu">
                            <div class="row">
                              @foreach($sub_sexual1 as $r)
                                <?php  
                                  $sub1_sexual = DB::table('categories')->whereIn('categories_id',[438,439])->where('sub_parent_id',$r->categories_id)->where('sub_parent_id','!=' , null)->where('parent_id','!=' , null)->where('sub_sub_parent_id',  null)->get();  
                                ?>
                              <div class="col-2">
                                <ul class="megamenu__links megamenu-links megamenu-links--root">
                                  <li class="megamenu-links__item megamenu-links__item--has-submenu"><a class="megamenu-links__item-link" href="{{url('doctor-list/'.$doctor->categories_id.'/'.$r->categories_id)}}">{{$r->sub_category_name}}</a>
                                    <ul class="megamenu-links">
                                      @foreach($sub1_sexual as $r1) {
                                        <li class="megamenu-links__item"><a class="megamenu-links__item-link" href="{{url('doctor-list/'.$doctor->categories_id.'/'.$r->categories_id)}}">{{$r1->sub_category_name}}</a>
                                        </li> 
                                      @endforeach 
                                    </ul>
                                  </li> 
            ...
            <div class="header__navbar-phone phone">
              <a href="tel:1800 060 0730" class="phone__body">
...
<!--                        <label class="location-list__item"> 
                            <span class="location-list__item-radio input-radio"> 
                                <span class="input-radio__body"> 
                                    <input class="input-radio__input form-control-select2" id="deliveryLocaion" name="location" {{strtolower(Session::get('set_location_name'))==strtolower(str_replace('"','',$r->city_name)) ?"checked":"" }} type="radio" value="{{str_replace('"','',$r->city_name)}}">  
